...
- Băm mô hình là chi phí áp đảo, không phải số học. Một lần `model_commitment` tốn **26.800 cycles cố định + 1.311 cycles mỗi tham số** (render số nguyên thành chuỗi thập phân rồi SHA-256), nên bốn lần gọi mỗi bước chiếm **76%** chi phí một bước ở `[4,64,2]`. Cách cắt **không** phải bỏ hash rồi mang giá trị sang — làm thế là mất ràng buộc giữa hash khai và trọng số thật. Cách đúng: bước đầu vẫn băm (neo vào start hash công khai), các bước sau `assert_model_eq` với mô hình ra của bước trước rồi mang hash sang. Đo thật trên lá `k=156`: **1,79×** rẻ hơn, cả cây 15 nút **1,45×**, public output khớp từng byte.

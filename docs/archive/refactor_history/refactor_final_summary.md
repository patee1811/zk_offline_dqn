...
- Phase 10: prepared release checklists, artifact release manifest, and release
  readiness checks.
- Phase 11A: cleaned safe generated local outputs and classified artifact
  cleanup policy.
- Phase 11B: classified remaining untracked artifact and fixture groups for
  release-owner decisions.
- Phase 11C: audited legacy script usage and recorded a non-destructive
  retirement plan.
- One-pass cleanup: archived verbose phase logs, removed the inactive `src/`
  skeleton, and deleted local/cache clutter without changing verifier behavior.
...
- Legacy script users are now documented at script/folder level.
...
- Migrate direct legacy script users before considering retirement.

...
| Multi-step training fragment | yes | no committed proof provenance yet | Python reference and SP1 backend support canonical k={1, 4, 8, 32, 128} tiny MLP vectors with deterministic sampling, replay membership, forward TD, SmoothL1 loss, backprop, SGD, target sync, and checkpoint chaining | Not full training, Adam, all replay batches, model selection, or recursive aggregation. Mark a k value proof-backed only when required SP1 provenance artifacts exist. |

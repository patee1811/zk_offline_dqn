...
loss~\cite{huber1964robust, girshick2015fastrcnn}:
...
of its two children. The root $\mroot$ serves as a compact commitment to all
...
operate on these integer values. A product is rescaled by truncation
\emph{toward zero}, $a \odot b = \operatorname{trunc}(ab / \fpscale)$, not by
flooring; the two agree only when $ab \ge 0$, and TD errors are negative
about as often as not. The discount factor enters the relation as the
integer $\gamma_{\mathrm{fp}} = 990$, so that $\gamma = \gamma_{\mathrm{fp}} / \fpscale$.
This eliminates floating-point
nondeterminism between Python semantic oracles and Rust/SP1 guests, so the same
inputs produce identical outputs across both verification paths.

The encoding has a ceiling, and it is close enough to matter. The guest
computes in \texttt{i64}, so the discount multiplication $\gamma_{\mathrm{fp}} \cdot Q$
leaves the representable range once the fixed-point $|Q|$ passes
$\texttt{i64::MAX} / \gamma_{\mathrm{fp}} \approx 9.3 \times 10^{15}$. Offline DQN can reach
that: value estimates grow multiplicatively when the target network is
refreshed too often. \Cref{thm:value-bound} states what the relation does about
it. The scale factor also decides which learning rates exist at all --- a rate
must be a multiple of $1/\fpscale$, so $10^{-2}$ encodes to $10$ and
$3 \times 10^{-4}$ encodes to $0$.

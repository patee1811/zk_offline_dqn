...
\begin{enumerate}[leftmargin=1.8em]
  ...
  times of $0.054$--$0.130$\,s and proof sizes of $1.27$--$2.85$\,MB across
  twenty-five of the twenty-six proved rows, naming the Groth16 variant as the
...
\textbf{Recursion off the GPU.} Recursive child-proof verification
inside the aggregate SP1 guest is implemented and measured for
$T \in \{16, 32, 64\}$ as flat chains and for binary trees to $T = 4{,}992$,
which removes the need for external child-proof verification at those sizes.
Two limits remain. Peak device memory is flat in $T$, so trace length is bounded
by budget rather than by memory; what is untested is a flat chain beyond
$T = 64$ and any recursion at $T = 128$.
...
\Cref{sec:discussion} measures the gap: at a matched step count, a network
carrying a MinAtar DQN's parameter count costs $126\times$ the cycles of the
one proved here, which puts a $k = 156$ leaf near $62$ billion cycles.

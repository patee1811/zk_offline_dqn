...
process. Unlike supervised learning, where a fixed
...
In the \emph{offline} setting~\cite{levine2020offline}, a policy is trained
entirely from a fixed replay
...
Zero-knowledge proof systems answer this directly: a prover can
...
virtual machine, which brings \emph{cryptographically} verifiable RL training
within reach. We keep that adjective throughout, because the phrase ``verifiable
reinforcement learning'' now most often denotes RL with verifiable rewards,
where an external checker scores an answer and nothing is proved to a third
party. That line and this one share a word and no threat model: there, the
verifier is the training signal; here, the verifier is an adversary's
counterparty who receives a proof and holds no trust in the prover.
...
Our starting point is that RL does not merely add another model class to this
literature. A recent survey of zero-knowledge verifiable machine
learning~\cite{zkmlsurvey2026} states its own scope plainly: it focuses on
supervised learning, ``as it is the most common form of ML and serves as the
foundation for the most of existing zero-knowledge-based verifiable machine
learning research.'' The training systems it covers are for linear and logistic
regression, support vector machines, $K$-means, decision trees and deep
networks. The omission is not an oversight in the survey; it reflects the
literature. And the gap persists not because value-based RL is a larger
computation, but because it carries three verification obligations that
supervised proof-of-training constructions either never encounter or encounter
in a weaker form:
...
\begin{enumerate}[leftmargin=1.8em]
  ...
  \item \textbf{Target-network synchronization is a discrete event.} The
  target is refreshed every $N$ steps. A prover free to move those events
  changes what every subsequent label means, yet the resulting run remains
  internally consistent.
  \item \textbf{Replay sampling draws with replacement from a buffer it never
  exhausts.} This is the obligation supervised constructions do meet, and the
  difference is worth stating precisely rather than claiming they ignore it.
  Kaizen~\cite{kaizen2024} commits the dataset and fixes each epoch's order
  with a permutation derived publicly from an epoch index and a committed seed,
  and its circuit checks that the batches partition the data; a prover who
  grinds that seed changes only the \emph{order} in which records are spent,
  because every record is spent exactly once. Offline DQN has no epoch and no
  partition. Indices are drawn with replacement, and a $1{,}248$-step run at
  batch size one touches $1{,}248$ of $50{,}000$ rows, so grinding the draw
  changes \emph{which} rows the claim is about --- it can select the $2.5\%$ of
  the buffer on which the policy looks best and never mention the rest. The
  obligation is therefore not order integrity but selection integrity, and it
  has to be discharged by deriving the draw from the commitment rather than by
  fixing a permutation over it.
\end{enumerate}
...
every chunk of a $1{,}248$-step run drew the same eight transitions from a
...
grind the seed, a bias vector first identified by Tan et
al.~\cite{zkpotvicinity2025}.

The contributions of this work are as follows:
\begin{enumerate}[leftmargin=1.8em]
  \item \textbf{RL-specific verification relations.} We formalize and implement
  relations for the three obligations above---model-generated labels,
  synchronization timing, and sampling integrity---together with replay
  ...
  \item \textbf{A whole training run under a single proof.} We prove three
  complete runs, each as one root proof over a binary tree of $156$-step leaves
  with every leaf bound to the committed dataset the reported policy was
  trained on: $1{,}248$ steps for each environment, and $4{,}992$ steps over
  $32$ leaves on \texttt{lunarlander-random}. All three carry
  $\mathrm{step\_start}=0$ and verify in $0.054$\,s, and the longest is the
  configuration \Cref{tab:rl-performance} reports
  ...
  sketch proofs for ten statements spanning membership, dataset commitment,
  sampler binding, bounded fixed-point arithmetic, Bellman targets, update
  correctness, chain and aggregation soundness, and the zero-knowledge boundary,
  each mapped to relation code, SP1 backend, tests and benchmark rows; and we
  report tamper rejection across $19$ adversarial
...
CartPole run long enough to learn well would need roughly $33$ GPU-hours of
node proving at the leaf size used here, and about $52$ once the binary tree is
rounded up to a power-of-two leaf count. These boundaries are stated in
\Cref{sec:discussion} and in the formal theorem statements.

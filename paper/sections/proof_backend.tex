...
\emph{before} the multiplication by $\gamma$, which is where \texttt{i64}
overflows. A case may not declare a bound the arithmetic cannot honour: the
guest rejects any \texttt{q\_abs\_max\_fp} exceeding
$\texttt{i64::MAX} / \gamma$. Cargo's release profile leaves overflow checks

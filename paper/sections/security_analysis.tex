For SP1-backed relations, soundness reduces to two assumptions: the SP1 proof
system soundly proves execution of the guest program, and SHA-256 commitments
are collision resistant for the artifact sizes considered. Under these
assumptions, an accepting proof implies that the guest relation accepted the
...
For distinct minibatch TD, the guarantee extends itemwise and also covers batch
structure. The public ordered index list must match the private items, duplicate
indices are rejected, and the claimed batch loss must equal the integer average
of recomputed per-item losses. This blocks attacks that replace one item,
shuffle items while preserving a multiset, or adjust the claimed average.

For model-grounded forward-TD SP1 cases, the selected Q-values are no longer
...
For the tiny one-step SGD SP1 case, the guarantee additionally binds the

...
\subsection{Stages 1--3: Dataset Provenance Pipeline}
...
benchmark source (Minari/D4RL~\cite{minari2024, fu2020d4rl}).
...
\subsection{Stages 4--5: Relation Proofs}
...
\subsection{Stages 6--7: Fragment Composition and Aggregation}

\textbf{Stage 6: Training fragments.} Multi-step fragments compose $k$ one-step
updates, each verifying minibatch sampling, replay membership, TD computation,
backpropagation, the clipped SGD update, the checkpoint transition, and
target-network synchronization. The sample indices are derived from the dataset
commitment rather than read from the statement, so this stage fixes which
transitions the fragment trains on. Proof-backed sizes are $k \in \{1, 4, 8\}$
on the canonical vectors, $k = 1$ on each committed expert dataset, and
$k = 156$ as the leaf of the whole-run trees.

\textbf{Stage 7: Aggregation.} Two modes, and the difference is the point. A
chunk chain binds an ordered sequence of child proof \emph{manifests}
($T \in \{32, 64, 128\}$), checking manifest hashes, public-input consistency,
dataset and configuration commitments, chunk boundaries and checkpoint links ---
the child proofs themselves are verified outside the relation. Recursive
aggregation instead runs the SP1 verifier on each child inside the guest, so
acceptance establishes that the children are valid proofs rather than assuming
it. Recursion is measured for $T \in \{16, 32, 64\}$ flat and for binary trees
up to $T = 4{,}992$, and requires a CUDA prover.
...
composes multi-step proofs. Layer~5 aggregates over longer traces, either by
binding child manifests or by verifying the child proofs inside the guest.}

...
complete DQN training run in the sense of covering initialization, model
selection, or optimizer state; they do cover a complete run of the relation the
system implements, at the trace lengths reported in
\Cref{tab:whole-run}.
...
Merkle case and the 1k/10k/50k/100k dataset-size scaling cases.  It does not prove
...
For accepted $k$-step training-fragment proofs, the final checkpoint is
obtained by the implemented deterministic sequence of minibatch sampling,
replay membership, Bellman target computation, gradient clipping,
fixed-point SGD updates, checkpoint chaining, and target-network hard
synchronization. Proof-backed instances are $k\in\{1,4,8\}$ on the canonical
vectors, $k=1$ on each committed expert dataset, and $k=156$ as the leaf of the
whole-run trees.
\end{theorem}
\begin{proof}[Proof sketch]
The fragment relation derives the sample indices rather than reading them,
verifies replay membership for every opened transition, recomputes per-step
TD and update arithmetic, clamps each gradient component to the declared
bound, links online and target checkpoint hashes, and applies target-network
synchronization at the configured interval.  Tampering with a minibatch index,
global step, checkpoint, or target-sync event breaks the recomputed trace.
Fragments at $k\in\{16,32,128\}$ have execute-mode evidence only and are not
proof-backed here.
\end{proof}

\begin{theorem}[Sampler binding]
\label{thm:sampler-binding}
For an accepted fragment, the minibatch indices are a function of the committed
dataset root and the fragment's global step offset. The prover has no free
choice of which transitions the fragment trains on.
\end{theorem}
\begin{proof}[Proof sketch]
The relation recomputes
$\sigma = \mhash(\mroot \,\|\, \mathrm{global\_step\_start})$ and rejects any
case whose published \texttt{sampler\_seed} differs, then derives each index
from $\sigma$ and the step position.  A prover who wants different indices must
present a different $\mroot$, which is a different dataset commitment and so a
different statement.  This closes a freedom that a valid proof does not
otherwise constrain: with a seed supplied as an input, a prover could search
seeds and publish only the fragment whose sampled transitions suited it, and
the proof would verify.  The residual freedom is grinding $\mroot$ itself, which
requires re-running the audited collection and commitment pipeline.  The
statement covers index selection only; it does not make the sampled subset
representative of the dataset.
\end{proof}

\begin{theorem}[Bounded fixed-point arithmetic]
\label{thm:value-bound}
For an accepted fragment, every $Q$ value, target value, and TD target lies
within the bound \texttt{q\_abs\_max\_fp} published in the statement, and that
bound is small enough that the discount multiplication cannot overflow the
\texttt{i64} encoding.
\end{theorem}
\begin{proof}[Proof sketch]
The guest asserts $|Q| \le B$ on both operands before forming
$\gamma \cdot Q / \fpscale$, asserts $|y| \le B$ on the result, and rejects any
case declaring $B > \texttt{i64::MAX} / \gamma_{\mathrm{fp}}$.  The order matters: a check
placed after the product is reached only once the product has already
overflowed.  Without these checks the multiplication wraps silently, and a
prover computing its witness under the same \texttt{i64} semantics obtains a
guest that accepts, so the proof attests to wrapped arithmetic.  Our own runs
detected this only because the Python oracle uses arbitrary-precision integers
and therefore disagreed.  The bound is a declared property of the statement
rather than a property of the host integer width, so it survives a change of
encoding.
\end{proof}

\noindent\emph{Remark.} Constructions that arithmetize training over a prime
field choose the field wide enough for the range they expect and are done;
Kaizen~\cite{kaizen2024} does exactly this. The statement above is not a
replacement for that choice and does not improve on it. What it adds is
specific to value learning: the range a value-based run will occupy is not
known in advance, because offline $Q$-learning diverges on some datasets and
the divergence is exponential rather than gradual. A width chosen for ``the
expected range'' is therefore an assumption about the training run, not about
the encoding, and it is the assumption most likely to be silently violated by
the very runs a regulator would want to inspect. \Cref{thm:value-bound} moves
that assumption out of the parameter choice and into the public statement, so
the verifier reads the admitted range instead of trusting that one was picked
correctly --- and an accepted proof carries the evidence that the run stayed
inside it.

\begin{theorem}[Chunk-chain and recursive aggregation soundness]
...
checkpoint links.  For accepted recursive instances with $T\in\{16,32,64\}$ under a
flat topology, and $T \in \{16, 1{,}248, 4{,}992\}$ under a binary-tree
topology, the aggregate additionally establishes that each child proof verifies
under the expected child verifying key.
\end{theorem}
\begin{proof}[Proof sketch]
...
In recursive mode the guest additionally runs the SP1 proof verifier on each
child proof against the expected child verifying key, so acceptance implies the
children are valid proofs rather than merely well-formed manifests.  This
strengthens the statement from binding external verification to establishing it
in-circuit.  The two modes are separate: the $T\in\{32,64,128\}$ rows remain
manifest chains, and recursive instances are established only at the sizes
listed above.  Recursive proving requires a CUDA device; it is not a soundness
condition but a reproduction requirement.  $T=128$ recursion is untested.
\end{proof}
...
lists, for each theorem, the relation, backend implementation, tests, and the
benchmark evidence in \Cref{tab:proof-cost} and \Cref{tab:tamper-summary} that

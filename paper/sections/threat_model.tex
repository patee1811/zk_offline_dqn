...
\begin{enumerate}[leftmargin=1.8em, label=\textbf{A\arabic*}.]
  ...
  \item \textbf{Sampler grinding}: rather than alter an index, choose the seed
  the indices are derived from. The adversary runs the fragment under many
  seeds and publishes only the one whose sampled transitions suit it. Every
  index is then consistent with the seed and every proof verifies, so this is
  invisible to A3; what it degrades is the meaning of the claim, from training
  on the committed dataset to training on a subset the prover chose from
  it~\cite{zkpotvicinity2025}.
  \item \textbf{Arithmetic overflow}: drive the fixed-point $Q$ values past the
  \texttt{i64} range so that the discount multiplication wraps. A prover
  computing its witness under the same wrapping semantics obtains a guest that
  accepts, and the proof then attests to arithmetic that means nothing.
...
A4 and A5 differ in kind from the rest. The others alter a value the relation
recomputes, so a correct relation rejects them and a tamper suite can exercise
them by mutating a field. These two are exercised by supplying inputs that are
internally consistent, which is why they were closed by constraining the
statement --- deriving the seed, publishing a bound --- rather than by adding a
check on a witness.
...
source-integrity checks that were performed. Public Minari/D4RL~\cite{minari2024, fu2020d4rl} benchmark rows are source-integrity commitments,
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Recursive aggregation on a CPU prover, or at $T = 128$; and PLONK
  child proofs, which are untested.
...
Chains are local; initialization is an input \\
\midrule

...
from scripts.artifacts_export.verify_forward_td_mlp_test_vector import (  # noqa: E402
    assert_equal,
...
def verify_tensor_pack(claimed: Dict[str, Any], expected: Dict[str, Any], label: str) -> None:
...
def verify_vector(vector: Dict[str, Any]) -> Dict[str, Any]:
    ...
    assert_equal(public["network_spec_hash"], build_network_spec_hash(public["network_layer_sizes"], fp_scale), "network_spec_hash")
    assert_equal(public["pre_model_commitment"], build_model_commitment(pre_model, fp_scale), "pre_model_commitment")
    assert_equal(public["post_model_commitment"], build_model_commitment(post_model, fp_scale), "post_model_commitment")
    assert_equal(public["target_model_commitment"], build_model_commitment(target_model, fp_scale), "target_model_commitment")
    ...
    verify_forward_trace(claimed_forward["online_obs"], expected_forward["online_obs"], "online_obs")
    verify_forward_trace(claimed_forward["online_next"], expected_forward["online_next"], "online_next")
    verify_forward_trace(claimed_forward["target_next"], expected_forward["target_next"], "target_next")
    for key in [
        "q_online_action_fp",
        "next_action_online",
        "q_target_max_fp",
    ...
    assert_equal(update_witness["smooth_l1_grad_fp"], expected["smooth_l1_grad_fp"], "smooth_l1_grad_fp")
    verify_tensor_pack(update_witness["gradient_tensors"], expected["gradient_tensors"], "gradient_tensors")
    verify_tensor_pack(update_witness["delta_tensors"], expected["delta_tensors"], "delta_tensors")
...
def main() -> None:
    ...
    print("=== VERIFY ONE STEP SGD TINY TEST VECTOR ===")
    print("input_path =", args.input)
    print("schema_version =", vector["schema_version"])
    print("item_index =", result["index"])
    print("loss_fp =", result["loss_fp"])
    print("smooth_l1_grad_fp =", result["smooth_l1_grad_fp"])
    print("verification_passed = True")
    print("all_one_step_sgd_tiny_ok = True")

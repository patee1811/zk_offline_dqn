...
from zk_offline_dqn.backends.sp1.forward_td_mlp import BACKEND_DIR, DEFAULT_CASE_PATH, cargo_command, load_case, tampered_case, verify_case_reference

TAMPER_CASES = ["tamper_reward", "tamper_action", "tamper_done", "tamper_state", "tamper_next_state", "tamper_online_weight", "tamper_target_weight", "tamper_q_online_public", "tamper_q_target_public", "tamper_td_target_public", "tamper_td_error_public", "tamper_loss_public", "tamper_checkpoint_hash_public", "tamper_dataset_root_public", "tamper_manifest_hash_public", "tamper_audit_report_hash_public"]
...
def run_tamper_checks(out_dir: Path) -> Dict[str, Any]:
    ...
    report = {"relation": "forward_td_mlp", "all_passed": all(item["passed"] for item in checks), "checks": checks}

...
from zk_offline_dqn.backends.sp1.short_trace import BACKEND_DIR, DEFAULT_CASE_PATH, cargo_command, load_case, tampered_case, verify_case_reference

TAMPER_CASES = ["tamper_intermediate_checkpoint", "tamper_step_order", "tamper_gradient_inside_step", "tamper_new_weight_inside_step", "tamper_start_checkpoint_hash_public", "tamper_final_checkpoint_hash_public", "tamper_trace_hash_public", "tamper_num_steps_public"]
...
def run_tamper_checks(out_dir: Path) -> Dict[str, Any]:
    ...
    report = {"relation": "short_trace", "all_passed": all(item["passed"] for item in checks), "checks": checks}
